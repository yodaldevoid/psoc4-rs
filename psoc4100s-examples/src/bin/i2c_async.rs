...
use psoc4100s_hal::i2c::{self, Config, InterruptHandler};
use psoc4100s_hal::peripherals::SCB1;
use {defmt_rtt as _, panic_probe as _};

bind_interrupts!(struct Irqs {
    SCB_1_INTERRUPT => InterruptHandler<SCB1>;
});
...
async fn main(_spawner: Spawner) {
    ...
    info!("set up I2C");
    let mut i2c = i2c::I2c::new_async(p.SCB1, &i2c_clk_src, scl, sda, Irqs, Config::default());
    ...
    let mut val = 0xAA;
    loop {
        i2c.write(mcp23017::ADDR, &[GPIOA, val]).await.unwrap();
